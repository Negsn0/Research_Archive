...
function main()
    ...
    for A in (100:2:132)
        ...
        # 固有値と基底状態をファイルに出力
        open("0$(A)_Sn_Energy_Levels_with_Basis1.txt", "w") do file
            println(file, "Energy Levels and Basis States for A=$(A):")
            
            # 中性子
            println(file, "Neutron Levels (MeV):")
            for (energy, vec) in zip(vals_n, eachcol(vecs_n))
                max_idx = argmax(abs.(vec))  # 基底状態のインデックス
                n, l, j = basis_list_n[max_idx]  # 対応する基底状態を取得
            ...
            for (energy, vec) in zip(vals_p, eachcol(vecs_p))
                max_idx = argmax(abs.(vec))  # 基底状態のインデックス
                n, l, j = basis_list_p[max_idx]  # 対応する基底状態を取得
                println(file, "Energy: $(round(energy, digits=3)) MeV, Basis: (n=$(n), l=$(l), j=$(j))")
            end
            
        end
    end
    println("Finish Calicuration.")
    return nothing
end
