...
function f_lambda(λ, epsilons, Δ, n)
    sum = 0.0
    for ek in epsilons
        sum += 0.5*(1 - (ek - λ) / sqrt((ek - λ)^2 + Δ^2))
    end
    return sum - n
end
...
function main(A::Int)
    ...
    λ_sol = find_zero(
        ...
        (minimum(epsilons)-2.0, maximum(epsilons)+2.0)
    )
    ...
    reΔ = recal_D(epsilons, Δ, λ_sol, G)
    println("Recalculated Δ: ", reΔ)

    re_N = recal_N(λ_sol, epsilons, reΔ)
    differN = abs(re_N-n)
    println("Recalculated N: ", re_N)
    println("Target n: ", n)#=
    open("zSn_parameter$(A).txt","w") do file
        println(file,"Δ,G = $(Δ),$(G)")
...
# open("Snparameters.txt","w") do file
#         println(file,"A,λ")
#         for A in 100:2:132
#         try
#             Δ,G,λ_sol=main(A)
#             println(file,"$(A),$(λ_sol),$(Δ)")
#         catch e
#             println("Error encountered for A=$A: ", e)
#             # エラー内容をログファイルに保存したい場合は以下を追加
#             open("error_log.txt", "a") do file
#                 println(file, "Error encountered for A=$A: ", e)
#             end
#         end
#     end
# end
